...
CREATE index keys_public_key_idx on keys(public_key);
...
values
    select 
        issuer_id, issuer_type, created_at, expires_at, last_rotated_at, created_at,
...
values
    select 
        issuer_id, created_at, signing_key, issuer_cohort
    from issuers
;
